    ...
    try {
        Dexter.findPokemon(capitalizeFirstLetter(arg), function (p) {
            ...
            embed = new Discord.RichEmbed()
                ...
                .addField('Description', p.pokedex_entries.Y.en)
                .setColor('BLUE')
                .setFooter(`${config.name} v${config.version}`)
                .setTimestamp()
            message.channel.send({ embed });
            ...
            message.channel.awaitMessages(filter, {
                ...
                if (collected.first().content.toLowerCase ()) === "yes" || collected.first().content.toLowerCase ()) === "ja") return showInGerman(p, message);
              ...
              message.channel.send("There was no information that was given within the allotted amount of time. ");
            })
    })
} catch (e) {
    embed = new Discord.RichEmbed()
...
function showInGerman(p, message) {
    ...
    Dexter.findPokemon(capitalizeFirstLetter(arg), function (p) {
        ...
        .addField('Beschreibung', p.pokedex_entries.Y.de)
        .setColor('BLUE')
        .setFooter(`${config.name} v${config.version}`)
        .setTimestamp()
    message.channel.send({ embed });
